...
object DragUtils {
    ...
    fun registerSwipeAdd(
        ...
        layoutId: Int = R.layout.float_sidebar,
        ...
        if (slideOffset != -1f) {
            ...
            when (event.action) {
                MotionEvent.ACTION_DOWN -> {
                    Log.d("registerSwipeAdd","ACTION_DOWN")
                    downX = event.rawX
                }
                MotionEvent.ACTION_MOVE -> {
                    // 起始值小于最小边界值，并且当前偏离量大于最小边界
                    Log.d("registerSwipeAdd","ACTION_MOVE")
                    if (downX < start * screenWidth && offset >= start) {
                        val progress = minOf((offset - start) / (end - start), 1f)
                        setAddView(event, progress, listener, layoutId)
                    } else {
                        Log.d("registerSwipeAdd", "dismissAdd")
                        if (!isFixed) dismissAdd()
                    }
                }
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    Log.d("registerSwipeAdd","ACTION_UP")
                    downX = 0f

                    when (isFixed) {
                        true -> {
                            playCollpaseAnim(event, 1f, listener, layoutId)
                        }
                        false -> {
                            val progress = minOf((offset - start) / (end - start), 1f)
                            Log.d("registerSwipeAdd", progress.toString())
                            if (progress == 1f) {
                                Log.d("registerSwipeAdd", "go Fixed")
                                isFixed = true
                            } else {
                                playCollpaseAnim(event, progress, listener, layoutId)
                            }
                        }
                    }
                }
    ...
    private fun playCollpaseAnim(event: MotionEvent,
                                 progress: Float,
                                 listener: OnTouchRangeListener? = null,
                                 layoutId: Int = R.layout.float_sidebar)
    {
        val currentTranslationX = addView?.let {
            -it.width * (1 - progress)
        } ?: run {
            0f
        }
